...
import com.example.gd10_d_10920.databinding.ListDataMahasiswaBinding

//buat dulu adapternya untuk nampilin fragmentnya
    ...
    private val context: Context
        ...
        inner class MahasiswaViewHolder(item: ListDataMahasiswaBinding):RecyclerView.ViewHolder(item.root){
            ...
            fun bind(mahasiswaData: MahasiswaData){
                with(binding) {
                    //yg dikanan adalah nam penampungnya yg di MahasiswaData
                    txtNim.text = mahasiswaData.nim
                    txtNama.text = mahasiswaData.nama

                    //ini juka diklik datanya nanti muncul detailnya
        ...
        override fun onBindViewHolder(holder: MahasiswaViewHolder, position: Int) {
            holder.bind(listMahasiswa[position]) //posisi data resycel viewnya
        }

        override fun getItemCount(): Int = listMahasiswa.size //berapa data yang dimunculin
}